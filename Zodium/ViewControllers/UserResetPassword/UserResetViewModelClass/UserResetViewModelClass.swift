...
import Foundation
class UserResetViewModelClass{
    
    var delegate : ResponseProtocol?
    
    func callresetPasswordApi(param: [String:Any],onSucssesMsg:String){
        LoaderClass.shared.loadAnimation()
        APIClient.postAuth(url: .resetPassword, parameters: param, method: .post, contentType: .applicationJson, authorizationToken: true) { (response) in
            print(response)
